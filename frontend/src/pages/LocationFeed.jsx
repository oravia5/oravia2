...
export default function LocationFeed() {
  ...
  const fetchLocationPosts = async () => {
    ...
    try {
      ...
      const res = await client.get(`/posts/location-feed/${encodeURIComponent(decodedLoc)}`);
      if (res.data.success) {
        setPosts(res.data.data);
      }
    } catch (err) {
      console.error(err);
      setError('Failed to load posts from this location.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="location-feed-page animate-fade">
      {/* Header bar */}
      <header className="loc-header">
        <button className="back-btn" onClick={() => navigate(-1)} aria-label="Back">
          <ArrowLeft size={20} />
        </button>
        <span className="loc-header-title">{decodedLocationName}</span>
      ...
      <main className="loc-body">
        {loading ? (
          ...
          <div className="loc-container">
            {/* Top Stat Banner */}
            <div className="loc-banner-card">
              <div className="loc-icon-circle">
                <MapPin size={28} />
              </div>
              <div className="loc-meta-info">
                <h2>{decodedLocationName}</h2>
                <p className="posts-count-label">
                  <strong>{posts.length}</strong> {posts.length === 1 ? 'post' : 'posts'} tagged here
                </p>
              </div>
            </div>
            ...
            {/* Posts Content */}
            {posts.length === 0 ? (
              <div className="empty-loc-state">
                <span className="empty-emoji">📍</span>
                <h3>No posts here</h3>
                <p>Be the first to share a post at {decodedLocationName}!</p>
              </div>
            ) : viewMode === 'grid' ? (
              <div className="loc-posts-grid">
                {posts.map((post) => (
                  <div 
                    key={post._id} 
              ...
              <div className="loc-posts-list">
                {posts.map((post) => (
                  <PostCard key={post._id} post={post} onDeleteSuccess={fetchLocationPosts} />
                ))}
              </div>
            )}
          </div>
      ...
      <style>{`
        ...
        .loc-loader, .loc-error, .empty-loc-state {
          display: flex;
